...
class MyMontitorThread implements Runnable {
    ...
    public void run() {
        while (run) {
            System.out.println(String.format("[monitor] [%d/%d] active: %d, completed: %d" + " Task: %d, isShutdown: %s, isTerminoted: %s", this.executor.getPoolSize(), this.executor.getCorePoolSize(), this.executor.getActiveCount(), this.executor.getCompletedTaskCount(), this.executor.getTaskCount(), this.executor.isShutdown(), this.executor.isTerminated()));
...
public class BasicThreadPollExecutor {
    public static void main(String[] args) throws InterruptedException {
        ...
        MyMontitorThread moniterTask = new MyMontitorThread(pool, 3);
        Thread monitor = new Thread(moniterTask);
        monitor.start();
